...
`app.go` 构造数据库、平台能力和业务模块，组装 Registry、调度、事件投递、鉴权、AI、HTTP 与前端资源，管理 HTTP 服务的运行和退出，TLS 由外部反向代理终止；同时提供模块启停、外部模块接入/代理、备份和维护任务等平台入口。内置模块的额外路由和关闭通过生命周期/路由契约遍历，不按投资 ID 分支。
