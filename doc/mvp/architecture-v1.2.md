...
部署配置使用命令行参数，日常配置通过 `/settings` 页面管理，不读取配置文件。AI 环境变量仅作为尚未保存页面设置时的初始值；保存后使用 SQLite `workspace_settings`，新请求立即生效。API Key 保存在私有工作空间数据库中，不通过设置接口回显，也不写入日志；数据库和备份须按敏感文件保管。主题与动态效果同样持久化。修改密码验证旧密码并使已有会话失效。Docker 是未来部署适配器，不是本地 MVP 的依赖。
