...
| SSRF | 不提供通用 URL 抓取器；通知 action 只接受本站相对路由；Provider base URL 来自可信启动配置或经认证、CSRF 保护的单用户设置接口；仅允许 HTTP(S)，拒绝 URL 内凭据、查询参数与片段，禁止重定向 | 工作空间拥有者可主动配置内网模型服务；拥有者须信任所配置的端点，更换地址必须重新填写密钥；未来连接器仍需独立限定访问范围 |
...
| API Key/敏感提示泄露 | API Key 来自初始环境变量或设置页面，保存在权限为 `0600` 的数据库中；设置接口仅返回是否已配置，禁止缓存；连接测试错误不透传上游内容；Responses 使用 `store:false`；Tool 参数按敏感字段名脱敏 | 密钥未在数据库中额外加密，数据库及其备份必须限制访问；主机失陷不在应用层防护范围内；普通文本中的秘密无法可靠自动识别 |
