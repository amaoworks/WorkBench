...
import { overviewSchema } from "./queries";
export default function InvestmentWidget({ widget }: { widget: Widget }) {
  const overview = useQuery({ queryKey: ["widget", widget.id], queryFn: async () => overviewSchema.parse(await api<unknown>(widget.dataRoute)), refetchInterval: 60_000 });
