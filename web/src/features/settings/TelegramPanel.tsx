...
export function TelegramPanel() {
  ...
  return <div className="space-y-4">
    ...
    <div className="studio-form"><h3>最近投递状态</h3>
      ...
      {value.failedDeliveries > 0 && <p className="mt-2 text-danger">{value.failedDeliveries} 条投递已停止重试。请检查配置；触发内容仍可在站内查看。</p>}
    </div>
  </div>;
}
...
function TelegramForm({ value }: { value: TelegramSettings }) {
  ...
  const test = useMutation({ mutationFn: () => apiValidated("/api/settings/telegram/test", telegramTestSchema, { method: "POST", body: JSON.stringify(payload()) }),
    onSuccess: () => setTestResult("测试消息已发送，请在 Telegram 中查看。"), onError: (error) => setTestResult(error.message), onSettled: refresh });
  const busy = save.isPending || test.isPending;
  return <form className="studio-form" onSubmit={(event) => { event.preventDefault(); save.mutate(); }}>
    <div className="control-heading"><div><h3>Telegram 推送</h3><p>接收投资价格预警，关闭网页后仍可提醒。</p></div></div>
    <fieldset className="form-fields" disabled={busy} onChange={() => setTestResult("")}>
      <label className="check-option"><input type="checkbox" checked={enabled} onChange={(event) => setEnabled(event.target.checked)} />启用 Telegram 投资预警</label>
      <label className="studio-field"><span>Bot Token <span className="field-meta">{value.hasBotToken && !clearBotToken ? "已保存 · 留空保留" : "尚未设置"}</span></span>
        <input type="password" autoComplete="new-password" value={botToken} maxLength={512} onChange={(event) => { setBotToken(event.target.value); setClearBotToken(false); }} placeholder="从 @BotFather 获取" />
      </label>
      <label className="studio-field"><span>Chat ID</span><input aria-label="Chat ID" value={chatId} onChange={(event) => setChatId(event.target.value)} maxLength={64} placeholder="个人/群组数字 ID 或 @频道用户名" />
        <small>私聊请先向机器人发送 /start；群组请先添加机器人，频道需要授予发布权限。</small>
      </label>
      {value.hasBotToken && <label className="check-option"><input type="checkbox" checked={clearBotToken} onChange={(event) => { setClearBotToken(event.target.checked); if (event.target.checked) { setBotToken(""); setEnabled(false); } }} />清除 Token 并停用推送</label>}
    </fieldset>
    ...
    <p className="form-note">测试会向表单中的目标发送一条消息，不会保存配置。仅转发启用后新产生的投资预警；停用期间和超过 15 分钟的提醒不补发。密钥存于服务器数据库及备份中，页面不会回显。</p>
  </form>;
}
