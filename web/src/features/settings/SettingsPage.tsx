...
function PasswordForm({ mode }: { mode: string }) {
  ...
  if (mode === "local") return <div className="local-mode"><ShieldCheck size={32} strokeWidth={1.3} /><h3>当前使用本机免登录模式</h3><p>如需密码保护，请在启动时设置初始密码并使用 <code>-auth password</code>，之后即可在这里修改密码。</p></div>;
...
function DataPanel({ value }: { value: Settings }) {
  ...
  return <div className="studio-form"><div className="backup-panel"><div className="icon-tile"><Database size={22} /></div><div><h3>数据备份</h3><p>在线创建完整数据库备份，无需中断工作。</p></div><button className="studio-button secondary" disabled={backup.isPending} onClick={() => backup.mutate()}>{backup.isPending ? <LoaderCircle size={16} className="animate-spin" /> : <Database size={16} />}{backup.isPending ? "正在备份" : "创建备份"}</button></div>{file && <p className="inline-result" role="status"><Check size={16} /><span>已保存至数据库同目录的 backups/{file}</span></p>}<p className="form-note backup-note">备份保存在服务器，包含工作数据、设置及已保存的密钥，请妥善保管。</p>
    <section className="deployment-panel" aria-labelledby="deployment-title"><div className="control-heading"><h3 id="deployment-title"><Server size={17} />运行信息</h3><span className="deployment-hint">启动配置 · 只读</span></div><dl><div><dt>监听地址</dt><dd>{deployment.listenAddress}</dd></div><div><dt>数据库路径</dt><dd>{deployment.dataPath}</dd></div><div><dt>认证模式</dt><dd>{deployment.authMode === "password" ? "密码登录" : "本机免登录"}</dd></div><div><dt>应用 TLS</dt><dd>{deployment.tls ? "已启用" : "未启用（外部 HTTPS 可由反向代理提供）"}</dd></div><div><dt>允许访问的 Host</dt><dd>{deployment.allowedHosts?.length ? deployment.allowedHosts.join(", ") : "默认允许本机监听地址"}</dd></div></dl><p className="form-note">端口使用 -listen，访问域名使用 -allowed-host，数据路径使用 -data。修改这些启动参数后需重启服务。</p></section></div>;
}
