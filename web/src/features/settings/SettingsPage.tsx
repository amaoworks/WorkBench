...
function AIForm({ value }: { value: AISettings }) {
  ...
  const save = useMutation({ mutationFn: () => api("/api/settings/ai", { method: "PUT", body: JSON.stringify(payload()) }), onSuccess: async () => { setApiKey(""); toast.success("AI 配置已保存"); await client.invalidateQueries({ queryKey: ["settings"] }); await client.invalidateQueries({ queryKey: ["ai"] }); }, onError: (err) => toast.error(err.message) });
  ...
  return <form onSubmit={(event) => { event.preventDefault(); save.mutate(); }} className="studio-form">
    <div className="control-heading"><div><h3>AI 服务</h3></div><Switch label="启用 AI 服务" checked={enabled} disabled={busy} onChange={setEnabled} /></div>
    <fieldset disabled={busy} className="form-fields" onChange={() => setTestResult("")}>
      <label className="studio-field"><span>接口地址 <span className="field-meta">BASE URL</span></span><input type="url" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder="https://api.openai.com/v1" required maxLength={2048} /></label>
      ...
      <label className="studio-field"><span>访问密钥 <span className="key-status"><KeyRound size={12} />{value.hasApiKey && !clearApiKey ? "已保存" : "未设置"}</span></span><input type="password" autoComplete="new-password" value={apiKey} onChange={(e) => { setApiKey(e.target.value); setClearApiKey(false); }} placeholder={value.hasApiKey && !clearApiKey ? "留空保持不变" : "输入 API Key"} maxLength={8192} /></label>
      {value.hasApiKey && <label className="check-option"><input type="checkbox" checked={clearApiKey} onChange={(e) => { setClearApiKey(e.target.checked); if (e.target.checked) { setApiKey(""); setEnabled(false); } }} />清除密钥并停用</label>}
    </fieldset>
...
function PasswordForm({ mode }: { mode: string }) {
  ...
  if (mode === "local") return <div className="local-mode"><ShieldCheck size={32} strokeWidth={1.3} /><h3>本机免登录模式</h3></div>;
  return <form className="studio-form" onSubmit={submit}><div className="control-heading"><div><h3>修改登录密码</h3></div><ShieldCheck size={24} className="muted-icon" /></div><fieldset className="form-fields" disabled={change.isPending}>
    ...
    </fieldset><div className="form-actions"><button className="studio-button" disabled={change.isPending || !currentPassword || !longEnough || categories < 3 || newPassword !== confirm}>{change.isPending ? <LoaderCircle size={16} className="animate-spin" /> : <KeyRound size={16} />}更新密码</button></div></form>;
}
...
function AppearanceForm({ value }: { value: Settings }) {
  ...
  return <div className="studio-form"><div className="control-heading"><div><h3>空间色调</h3></div></div><div className="theme-options" role="group" aria-label="空间色调">{themes.map(({ id, title, subtitle, icon: Icon }) => <button key={id} className={`theme-option ${theme === id ? "selected" : ""}`} aria-pressed={theme === id} disabled={save.isPending} onClick={() => save.mutate({ theme: id, motion }, { onError: (err) => toast.error(err.message) })}><div className={`theme-preview preview-${id}`} aria-hidden="true"><div className="preview-rail" /><div className="preview-content"><i /><i /><div><b /><b /></div></div><span><Icon size={18} /></span></div><div className="theme-caption"><span>{title}<small>{subtitle}</small></span>{theme === id && <Check size={16} />}</div></button>)}</div><div className="motion-control"><div className="motion-label"><AudioLines size={20} /><div><h3>减少动态效果</h3></div></div><Switch label="减少动态效果" checked={motion === "reduced"} disabled={save.isPending} onChange={(checked) => save.mutate({ theme, motion: checked ? "reduced" : "full" }, { onError: (err) => toast.error(err.message) })} /></div></div>;
}

function DataPanel({ value }: { value: Settings }) {
  const [file, setFile] = useState("");
  const backup = useMutation({ mutationFn: () => apiValidated("/api/system/backup", backupSchema, { method: "POST", body: "{}" }), onSuccess: (result) => { setFile(result.file); toast.success("备份已创建"); }, onError: (err) => toast.error(err.message) });
  const deployment = value.deployment;
  return <div className="studio-form"><div className="backup-panel"><div className="icon-tile"><Database size={22} /></div><div><h3>数据备份</h3></div><button className="studio-button secondary" disabled={backup.isPending} onClick={() => backup.mutate()}>{backup.isPending ? <LoaderCircle size={16} className="animate-spin" /> : <Database size={16} />}{backup.isPending ? "正在备份" : "创建备份"}</button></div>{file && <p className="inline-result" role="status"><Check size={16} /><span>已保存至 backups/{file}</span></p>}
    <section className="deployment-panel" aria-labelledby="deployment-title"><div className="control-heading"><h3 id="deployment-title"><Server size={17} />运行信息</h3><span className="deployment-hint">只读</span></div><dl><div><dt>监听地址</dt><dd>{deployment.listenAddress}</dd></div><div><dt>数据库路径</dt><dd>{deployment.dataPath}</dd></div><div><dt>认证模式</dt><dd>{deployment.authMode === "password" ? "密码登录" : "本机免登录"}</dd></div><div><dt>外部访问地址</dt><dd>{deployment.publicUrl || "本机 HTTP 服务"}</dd></div><div><dt>允许访问的 Host</dt><dd>{deployment.allowedHosts?.length ? deployment.allowedHosts.join(", ") : "默认允许本机监听地址"}</dd></div></dl></section></div>;
}
